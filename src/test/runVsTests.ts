...
import { mkdirIfDoesNotExist } from './suite/asyncfs';
...
async function runTestsInEmptyWorkspaceFolder(options: TestOptions, version: string, launchArgs: string[]): Promise<number> {
	...
	const workspaceFolderName = `java-refactoring-test-workspace-folder_${version}_`;
	...
	await mkdirIfDoesNotExist(path.join(userDataDir.path, 'User'), { recursive: true });
